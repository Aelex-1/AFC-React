 ...
 */
function AccordionTrigger({ children, onClick}: AccordionTriggerProps) {
  return (
    <summary
      className="flex w-full cursor-pointer items-center justify-between py-4 text-lg font-semibold text-zinc-900 group-open:text-(--color-secondary) group-open:font-bold transition-colors duration-300"
      onClick={onClick}
    >
      {children}
      <MdArrowDropDown className="text-2xl shrink-0 transition-transform duration-300 group-open:rotate-180"/>
    </summary>
  );
}
...
function AccordionContent({ children, className }: AccordionContentProps) {
  return (
    <div className={`pb-6 text-sm leading-5 tracking-normal text-zinc-900 ${className}`}>
      {children}
    </div>
  );
}
